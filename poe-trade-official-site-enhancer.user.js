...
// @require      https://cdn.bootcss.com/twitter-bootstrap/4.2.1/js/bootstrap.min.js
...
"https://cdn.bootcss.com/twitter-bootstrap/4.2.1/js/bootstrap.min.js"*/
var settingsIcon;
...
var settings = {
  ...
  enableChaosEquiv: {
    ...
    feature:`Sort item and currency searches on <b>poe.trade</b> by current chaos equivalent using <b>poe.ninja</b> values.`, showcase: 'https://raw.githubusercontent.com/ghostscript3r/poe-trade-official-site-enhancer/master/images/sort-real-curr-off.gif'
  },
  enableCurrencyLoad: {
    ...
    $.each( data.more, function( index, id ) {
      ...
      if (_.has(item, "listing.price.amount") && item.listing.price.amount) {
        ...
        spn.on('click', function(e) {
          e.preventDefault();
          fallbackCopyTextToClipboard(item.listing.whisper);
...
function addMaxQtCostToItem(data) {
  if (data.listing.price && data.listing.price.amount && !data.item.corrupted && !data.item.duplicated && data.item.category.gems) {
...
var renderChaosEquiv = function(item, data, localId, resultId) {
  if (item.find('.chaosEquiv').length == 0 && data.chaosEquivalent && data.listing.price) {
    ...
    if (data.listing.price.amount) {
      ...
      tip = getChaosEquivTip(data.buyout, data.listing.price.currency);
      item.find('div.price [data-field=price]').last().after(sortHandle);
      sortHandle.wrap(`<div class="gs-wrapper"></div>`);
      // if (data.listing.price.currency == "chaos") {
      //   item.find('.price-label ~ br').nextAll('span:not(.chaosEquiv)').hide();
    ...
    app.$nextTick(function(){
      ...
      .then(function() {
        ...
        $.each( Object.keys(allItems), function( index, itemId ) {
          allItems[itemId] = enhanceItem(allItems[itemId], index, allItems);
        });
...
var openSearch = function(search, newTab, target) {
  var st = $.extend({}, app.$store.state, {
    persistant: {
      ...
      name: _.has(search, "query.name.option") ? search.query.name.option : null,
      type: _.has(search, "query.type.option") ? search.query.type.option : null,
...
var openMultiSearch = function(searches) {
  ...
  .then(function() {
    ...
    app.$store.watch(function (state, getters) {
      debug("watching...", arguments);
      return state.transient.search.active.live;
    }, function(newVal, oldVal) {
      log("CHANGED!", arguments);
      if (connections.length && !newVal) resetConnections();
    });
  }, function() {
...
//function override
  ...
  function getChaosEquivTip(buyout, currency) {
...
var loadMultiSearches = function() {
