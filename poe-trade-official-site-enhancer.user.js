  ...
  <div class="container-fluid pt-1">
    ...
    <div class="tab-content" id="myTabContent">
      <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
        <div class="">
          <h2 class="">Poe Trade Official Site Enhancer <small class="text-secondary font-italic font-weight-light">v. 1.5.3</small></h2>
...
var renderChaosEquiv = function(item, data, localId, resultId) {
  if (item.find('.chaosEquiv').length == 0 && data.chaosEquivalent && data.listing.price) {
    ...
    if (data.listing.price.amount) {
      //TIP: SINGLE ITEM
      sortHandle = $(`<span class="chaosEquiv"><span class="currency currency-chaos"><b>${poe2?'Ex':'Chaos'} Equiv: </b>${(Math.round(data.chaosEquivalent*100)/100)}×</span><span class="currency-text currency-image"><img src="https://web.poecdn.com/image/Art/2DItems/Currency/CurrencyRerollRare.png?v=c60aa876dd6bab31174df91b1da1b4f9" alt="chaos" title="chaos"></span></span>`);
...
var initModSelector = function() {
  if (getSetting('useSaveManager') || getSetting('useModSelector')) {
    ...
    var modsIcon = $(`<span class="gs-style gs-fixTips gs-mods-selector" data-tippy="Filter selector"><i class="fas fa-filter"></i></span>`);
    var svicon = $(`<span class="gs-style gs-fixTips gs-mods-save" data-tippy="Save filter group"><i class="fas fa-save"></i></span>`);
    var ldicon = $(`<span class="gs-style gs-fixTips gs-mods-load" data-tippy="Load filter group"><i class="fas fa-folder-open"></i></span>`);
            ...
            if (_.has(app.$store.state.persistent, "filters.type_filters.filters.category.option") ) {
              ...
              switch (selectedType) {
                ...
                case "armour.focus":
                  ...
                  break;
                case "armour":
                  iframe.data('filterType', ['helmet', 'chest', 'gloves', 'boots', 'quiver', 'shield', 'focus']);
                  break;
                case "armour.buckler":
            ...
            modRow = $(/* html */ `<div class="affixRow" data-name="${modKey}">
              <div class="affix container-fluid">
                <div class="row affixName selectableAffix" id="${modRowKey}">
                  <div class="col-1"><a href="#" onclick="return false;" class="tier-toggler"><i class="fas mt-1 fa-caret-right"></i><i class="fas mt-1 fa-caret-down"></i></a></div>
...
var loadTypeaheadModsSearches = function(event, handle) {
  ...
  .append(`<div>type: ${query.type}${((query.disabled)?' (disabled)':'')}</div>`);
  $.each( query.filters, function( i, filter ) {
